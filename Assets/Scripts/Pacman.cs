...
public class Pacman : MonoBehaviour
{
    ...
    [SerializeField] private Animator animFish;
    [SerializeField] private Material playerMatDefault, playerPowerMat, playerInvinsMat;
    [SerializeField] private SkinnedMeshRenderer PlayerRend { get; set; }
...
//Private variables
    ...
    private void Awake()
    {
        animFish.Play("Swim", -1 , 1);
        animFish = GetComponent<Animator>();
        SkinnedMeshRenderer playerMesh = GetComponentInChildren<SkinnedMeshRenderer>();
        if (playerMesh != null)
        {
            PlayerRend = playerMesh;
            playerMatDefault = PlayerRend.material;
        }
    ...
    {                
        ...
        {
            PlayerRend.material = playerPowerMat;
        }
        else
        {
            PlayerRend.material = playerMatDefault;
        }
        //Respawn timer
        if (respawnTimer > -1)
        {
            respawnTimer += Time.deltaTime;
            if (PlayerRend.material != playerInvinsMat)
            {
                PlayerRend.material = playerInvinsMat;
            }
        ...
        {
            animFish.SetBool("SwimToggle", true);
        }
        else
        {
            animFish.SetBool("SwimToggle", false);
        }
        ...
        if (respawnTimer == -1)
        {
            ...
            if (currentLives > 0)
            {
                ...
                if(currentLives < lifeIcons.Length)
                {
                    ...
                    Debug.Log(lifeIcons[currentLives].name);
                }
                else
                {
